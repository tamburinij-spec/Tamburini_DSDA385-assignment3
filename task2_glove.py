"""
Task 2 – Machine Translation  |  GloVe Pretrained Embeddings
=============================================================
Trains a GRU encoder-decoder on the spa-eng.txt dataset
using frozen GloVe 100-d embeddings on the encoder side.
...
from models.gru_model  import GRUEncoder, GRUDecoder
from embeddings.glove  import load_glove_embeddings
from embeddings.one_hot import tokenize, build_vocab   # vocab helpers are shared
...
MAX_PAIRS       = 8000
MAX_LEN         = 12
EMBED_SIZE      = 100     # must match GloVe file
HIDDEN_SIZE     = 256
EPOCHS          = 20
BATCH_SIZE      = 64
TEACHER_FORCING = 0.5
...
glove_matrix = load_glove_embeddings(GLOVE_PATH, eng_w2i, embed_size=EMBED_SIZE)
print("GloVe loaded.")
...
model = Seq2SeqGRU(
    GRUEncoder(eng_sz, EMBED_SIZE, HIDDEN_SIZE, embedding_matrix=glove_matrix).to(device),
    GRUDecoder(spa_sz, EMBED_SIZE, HIDDEN_SIZE).to(device),
).to(device)
...
print(f"\nFinal BLEU (GRU + GloVe): {final_bleu:.4f}")
